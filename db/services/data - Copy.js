...
const ROLE_PAGES = {
  ...
  'harvesting-leader': ['stock-movements', 'material-requests'],
  'sawmill-leader':    ['stock-movements', 'material-requests'],
  'vat-leader':        ['stock-movements', 'material-requests'],
  'poles-leader':      ['stock-movements', 'material-requests'],
};
...
async function vatInboundList(userId) {
  ...
  const { rows } = await pool.query(
    `select st.id, st.item_description as product_size,
            st.requested_qty, st.received_qty,
            to_char(st.created_at,'DD/MM/YYYY') as requested_date,
            st.status,
            coalesce(
