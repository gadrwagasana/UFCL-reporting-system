...
router.get('/dashboard',   ADMIN_CEO_OPS, async (req, res) => {
  respond(res, await data.performanceDashboard(req.user.id), 120);
});

router.get('/departments', ADMIN_CEO_OPS, async (req, res) => {
  respond(res, await data.departmentScorecards(req.user.id), 120);
});

router.get('/trends',      ADMIN_CEO_OPS, async (req, res) => {
  respond(res, await data.performanceTrends(req.user.id), 120);
});
